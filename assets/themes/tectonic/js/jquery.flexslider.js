        ...
        setupPaging: function() {
          ...
          if (slider.pagingCount > 1) {
            for (var i = 0; i < slider.pagingCount; i++) {
              item = (vars.controlNav === "thumbnails") ? '<img src="' + slider.slides.eq(i).attr("data-thumb") + '"/>' : '<a><span>' + j + '</span></a>'; // [++] modification: span tag added
      ...
      if (!slider.animating && (slider.canAdvance(target, fromNav) || override) && slider.is(":visible")) {
        ...
        if (!fade) {
          ...
          if (!touch) {
			...
			if ($('html').hasClass('ie')) { // [++] modification: IE hack
				slider.slides.eq(slider.currentSlide).children('.flex-caption').hide();
			}
			
            slider.slides.eq(slider.currentSlide).fadeOut(vars.animationSpeed, vars.easing, function() {
				if ($('html').hasClass('ie')) { // [++] modification: IE hack
					$('.flex-caption', this).show();
				}
			});
  ...
  $.fn.flexslider = function(options) {
    ...
    if (typeof options === "object") {
      return this.each(function() {
		...
		// [++] modification: individual styling for captions and positioning caption blocks (except for gallery slider)
		if (!$this.hasClass('gallery')) {
			...
			$slides.each(function(i) {
				var $flexCaption = $slides.eq(i).children('.flex-caption'),
					bgColor = $flexCaption.attr('data-bg-color'),
					bgOpacity = $flexCaption.attr('data-bg-opacity'),
					align = $flexCaption.attr('data-block-align'),
					valign = $flexCaption.attr('data-block-valign'),
					width = $flexCaption.attr('data-width'),
					height = $flexCaption.attr('data-height'),
					bgRGBA;
				...
				if (bgColor || bgOpacity) {
					...
					if ($('html').hasClass('ie')) {
						...
						$flexCaption.css({'background-color': bgColor, 'background': 'transparent',
								'filter': 'progid:DXImageTransform.Microsoft.gradient(startColorstr=#' + hexBgOpacity + bgColor + ',endColorstr=#' + hexBgOpacity + bgColor + ')'});
					} else {
						$flexCaption.css({'background': bgRGBA});
					}				
				}
				
				// caption width and height
				if (width) {
					$flexCaption.css({'width': width});
				}
				if (height) {
					$flexCaption.css({'height': height});
				}
				
				// caption horizontal alignment
				if (align == 'right') {
					$flexCaption.css({'left': 'auto', 'right': '20px'});
				} else if (align == 'center') {
					if (!$('html').hasClass('ie7')) {
						$flexCaption.css({'left': '0', 'right': '0', 'margin-left': 'auto', 'margin-right': 'auto'});
					}
				}
				
				// caption vertical alignment
				if (valign == 'top') {
					$flexCaption.css({'bottom': 'auto', 'top': '20px'});
				} else if (valign == 'center' || valign == 'middle') {
					$flexCaption.css({'top': '50%', 'bottom': 'auto', 'margin-top': -parseInt($flexCaption.height()/2)});
				}
