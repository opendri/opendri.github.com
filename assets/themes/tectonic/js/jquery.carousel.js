...
;(function(window, document, $, undefined){
	...
	Plugin.prototype = {
		...
		init: function(){
			...
			$dom.slider.addClass(namespace + '-slider');
		...
		update: function(options){
			...
			this.$dom.slides.show(); // [++] show slides hidden by css
		...
		resize: function(){
			var containerWidth = this.$dom.frame.width(),
				containerHeight,
				slidesWidth = this.$dom.slides.width() + parseInt(this.$dom.slides.css('padding-right')), // [++]
				slidesHeight,
				sliderWidth = this.props.total * slidesWidth;
			...
			if (this.props.total > this.props.visible) {
				this.$dom.navContainer.css('display', 'block'); // [++]
			} else {
				this.$dom.navContainer.css('display', 'none'); // [++]
			}
		},
		...
		goTo: function(i, skipAnimation, touchend){
			var self = this,
				index = this._getValidatedTarget(i),
				cssPosition = this._getTargetPosition(index),
				duration = skipAnimation ? 0 : (touchend ? this.settings.animation.duration * 0.7 : this.settings.animation.duration),
				callback = function(){
				...
				prop, transitionProp, endEvent, transition, oldTransition, easeFunc;
...
//				handle.setAttribute('tabindex', 0);
		...
		_getTargetPosition: function(index){
			var slidesSize = this.$dom.slides.width() + parseInt(this.$dom.slides.css('padding-right')), // [++]
				prop = 'left',
				css = {};
		...
		_getVisibleSlides: function(){
			if (this.settings.visibleSlides > 0) {
				...
				var self = this,
					minSize = 0,
					containerSize = this.$dom.frame.width();
		...
		_shiftSlides: function(index, dir){
			...
			if ($slides.length > 0) {
				var axis = 'left',
					slideDimension = this.$dom.slides.width(),
		...
		_touchEnable: function(){
			...
			events['touchmove.'+namespace] = function(e) {
				...
				var sliderMinX = self.$dom.frame.width() - self.$dom.slider.width() + parseInt(self.$dom.slides.css('padding-right')); // [++]
				var sliderMaxX = 0; // [++]
				//console.log($.now()/1000+': start='+coords.start.x + ' end='+ coords.end.x + ' sliderX='+self.$dom.slider.position().left);

				if (Math.abs(distance.x) > Math.abs(distance.y)) {
					if (!(sliderOffset.left >= sliderMaxX && coords.end.x > coords.start.x) && !(sliderOffset.left <= sliderMinX && coords.end.x < coords.start.x)) { // [++]
					
						refDimension = sliderOffset.left;
			...
			events['touchend.'+namespace] = function(e) {
				...
				if (Math.abs(distance.x) > Math.abs(distance.y)) {
					...
					if (Math.abs(distance.x) > thresholds.distance || speed > thresholds.speed) {
						var refDimension = self.$dom.slides.width(),
					...
					self.goTo(targetSlide, false, true);
