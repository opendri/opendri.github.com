...
$(window).load(function() {
	...
	var $items_container = $('#portfolio-wrapper'),
		...
		ie_old = $.browser.msie && $.browser.version < 9 ? true : false;
	...
	if ($items_container.length && $items_filter.length) {
		...
		$(window).on('smartresize', function() {
			var col_margins = $(window).width() < 768 ? 0 : 20;
			var col_width = $items_container.children().filter(':first').width() + col_margins;
			$items_container.masonry({
		...
		$items_filter.on('click', 'a', function(e) {
			...
			if (select_filter == 'All' || $(this).parent().index() == 0) {		
				...
				$items_container.children().not('.' + select_filter).each(function(){
					if (!$(this).hasClass('hide')) {
						$(this).addClass('hide');
						if (!ie_old) {
				...
				$items_container.children('.' + select_filter).each(function(){
					if ($(this).hasClass('hide')) {
						$(this).removeClass('hide');
						if (!ie_old) {
							$(this).fadeIn('400', positionFooter);
